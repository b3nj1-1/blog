...
[LDAP](https://www.profesionalreview.com/2019/01/05/ldap/) es importante siempre escanear ldap porque este podria tener nombres, apellidos y direcciones que están escritos en formato de mapa DTMF aunque en este caso en especifico no lo utilizaremos, aqui les dejo la herramienta (Podríamos también escanear ldap con nmap):
```bash
...
Cuando vemos que carpetas esta compartidas procedemos a descargar todo el contenido que contienen las mismas en busca de posibles usuarios[recurse, prompt, mget](https://www.computerhope.com/unix/smbclien.htm):
...
Vemos el contenido de cada archivo en busca de patrones comunes entre archivos:
...
En algunos archivos encontramos nombres, la idea sería crear un diccionario en base a esos nombres que encontramos para posteriormente utilizarlo.
...
Sacamos los nombres de todos los archivo que habiamos extraido (Tener en cuenta que comúnmente en AD se utilizan nomenclaturas para los usuarios):
```bash
cat users.txt
Alexa Whitehat
Jack Goldenhand
...
Utilizamos este script para que nos automatice la parte de crear las nomenclaturas para cada usuario, simplemente es ejecutar el script, pasarle la lista de usuarios y listo este nos creara una lista de nombres de usuario con diferente nomenclatura que podemos utilizar para hacer bruteforcing:
...
Comprobaremos cuales usuarios de nuestra lista son válidos en el dominio mediante un ataque de fuerza bruta mediante la biblioteca Impacket con la herramienta [kerbrute](https://github.com/ropnop/kerbrute).
...
También les dejo todos los módulos disponibles que tiene[CrackMapExec Module Library](https://www.infosecmatter.com/crackmapexec-module-library/).
...
Para dar un poco de contexto sobre este archivo [¿Qué es VBScript?](https://www.taringa.net/+info/que-es-vbscript_12rc00).
...
Luego de analizar el script me encuentro con credenciales sin cifrar de un usuario valido en el dominio (El usuario a-whitehat fue uno de los que extrajo kerbrute):
...
Compruebo si las credenciales de este usuario que sacamos del script es correcto (Esto lo podemos ver con el signo + si fuera lo contrario daría un –):
